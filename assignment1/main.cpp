...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  ...
  while (getline(fin, line)) {
    ...
    // } 
    // std::cout << std::endl;
    struct Course course = {vec[0], vec[1], vec[2]};
...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  const std::string filename = "./student_output/courses_offered.csv";
  std::ofstream fout(filename);
  fout << "Title,Number of Units,Quarter\n";

  for (auto course: all_courses) {
    if (course.quarter != "null") {
      std::string title = course.title, unit = course.number_of_units, quarter = course.quarter;
      fout << title << "," << unit << "," << quarter << std::endl;
      deleted_courses.push_back(course);
    } 
  }
  
  for (auto course: deleted_courses) {
...
void write_courses_not_offered(std::vector<Course>& unlisted_courses) {
  /* (STUDENT TODO) Your code goes here... */
  const std::string filename = "./student_output/courses_not_offered.csv";
  std::ofstream fout(filename);
  fout << "Title,Number of Units,Quarter\n";
